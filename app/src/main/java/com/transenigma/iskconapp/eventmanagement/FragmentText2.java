...
public class FragmentText2 extends Fragment{
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        int[] ageArray = (int[]) bundle.getIntArray("ageArray");
        mChart.setData(generatePieData(ageArray));
        float a =.20f;
        ...
        for (int i=0;i<labels.length;i++){
            if(i<labels.length-1){
                labels[i]=labels[i] + "\t" + (40+i);
            }
        }
        l.setCustom(colorcodes, labels);
        l.setPosition(LegendPosition.BELOW_CHART_CENTER);
        mChart.getLegend().setWordWrapEnabled(true);
        return v;
    }
    protected PieData generatePieData(int[] ageArray) {

        int count = 4;

        ArrayList<Entry> entries1 = new ArrayList<Entry>();
        ArrayList<String> xVals = new ArrayList<String>();
        String[] lab = {"<20","20-35","35-50",">50"};

        for(int i = 0; i < count; i++) {
            if(ageArray[i]!=0){
                xVals.add(lab[i]);
                entries1.add(new Entry(ageArray[i],i));
            }
        }

        PieDataSet ds1 = new PieDataSet(entries1, "Age Wise "+name+" Distribution");
    ...
    private SpannableString generateCenterText() {
        SpannableString s = new SpannableString("Age Wise \n"+name+" Distribution");
